...
function applyTransform(element,transform){element.style.webkitTransform=transform
element.style.transform=transform}
var lv=document.getElementById("leftvert");var rv=document.getElementById("rightvert");var th=document.getElementById("tophorz");var bh=document.getElementById("bottomhorz");function resizeFrame(box,solid,fade){var verts=[lv,rv];var hrz=[th,bh];frm=[hrz,verts];for(var i=0;i<2;i++){var dim=(i!==0?height:document.body.clientWidth)/2;for(var j=0;j<2;j++){applyTransform(frm[i][j],composeTransform("translate",(i+1)%2,(j*2-1)*box[(i+1)%2],0,"px")) 
var grads=[frm[i][j].getElementsByClassName("first")[0],frm[i][j].getElementsByClassName("last")[0]];var mid=frm[i][j].getElementsByClassName("mid")[0];var midsize=(box[i]+(dim-box[i])*solid[i])/dim;applyTransform(mid,composeTransform("scale",i,midsize,1,""))
var gradscale=fade[i]*(dim-box[i])/dim;for(var k=0;k<2;k++){

 var translate=((midsize+(gradscale*.47)-.5)*dim)*(k*2-1);var transform=composeTransform("translate",i,translate,0,"px");transform+=" ";transform+=composeTransform("scale",i,gradscale,1,"");applyTransform(grads[k],transform)}}}}
...
function imageMover(x){var transform="translate3d(0, -"+x*(height+imgElement.clientHeight)+"px, 0)";imgContainer.style.webkitTransform=transform;imgContainer.style.transform=transform;}
